<template>
    <div v-loading.fullscreen.lock="loading">
        <div class="w cf">
            <div class="top">
                <h2>收付款
                    <span v-if="isShow == 1">审批</span>
                    <span v-else>查看</span>
                </h2>
                <el-button @click="model(0)" size="small" class="back">返回</el-button>
                <el-button v-if='isShow == 1' @click="model(1)" size="small" type="primary" class="sub1" :loading="isLoading">同意</el-button>
                <el-button v-if='isShow == 1' @click="model(2)" size="small" type="danger" class="sub2" :loading="isLoading">驳回</el-button>
            </div>
        </div>
        <div class="w">
            <div class="content" :style="{height:screenHeight}">
                ...
                <ul class="list cf">
                    ...
                    <li class="pt">
                        <span class="tit">待收付款</span>
                        <input class="ipt" type="text" v-model="amount" readonly>
                    </li>
                    <li class="pt">
                        <span class="tit">本次收付款</span>
                    ...
                    <li class="ptx">
                        ...
                        <textarea class="tex" v-model="discription" maxlength="50" readonly></textarea>
                    </li>
                    <li class="upload">
                ...
                <div class="line" v-if='isShow == 1'>
                    <span>审批意见</span>
                </div>
                <ul class="approval-opinion" v-if='isShow == 1'>
                    <li>
                        <span>可选审批意见</span>
                    ...
                    <li class="opinionItem">
                        <span>审批意见</span>
                            <textarea v-model="discription2" name="opinionItem" id="opinionItem" maxlength="50"></textarea>
                    </li>
                </ul>
            </div>
        </div>
            ...
            model(n){
                ...
                if(n == 0){
                    if(this.isShow == 1){
                        ...
                        this.$router.push({name:'payments',params:{debitId:this.id}})
                    }
                }else {
                    this.isLoading = true;
                    let message = ''
...
//            console.log(this.debitId);
            let params = new URLSearchParams();
            let url = addUrl.addUrl('approvalPayments')
            params.append('paymentId',this.debitId);
...
//                    console.log(response);
                    ...
                    this.amount = number.number(item.amount);
                    this.subjectName = item.subjectName
                    this.relationName = item.relationName

                    let payment = data.payment
                    this.money = number.number(payment.money)
                    this.businessDate = payment.businessDateYMD
                    this.discription = payment.discription
                    this.payType = String(payment.payType)
                    this.bankCode = payment.bankCode
                    this.attachUrlJson = JSON.parse(payment.attachUrl)
