                    ...
                    <el-table-column label="操作" align="center" width="60px">
                        <template slot-scope="scope">
                                <span class="operation">
                                    <router-link :to="{name:'intangibleAssets',params:{debitId:scope.row.idString}}" class="see">
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods:{
            model(){
                this.loading = true;
                var url = addUrl.addUrl('intangibleAssetsListCalculation')
                axios.post(url)
                    .then(response=> {
            ...
            axios(){
                var url = addUrl.addUrl('intangibleAssetsList')
                axios.post(url)
                    .then(response=> {
                        this.loading = false;
