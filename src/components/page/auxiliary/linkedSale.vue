        ...
        <div class="w cf">
            <div class="content" :style="{height:screenHeight}">
                ...
                <el-table :data="tableList" class="tableList" :show-header=false >
                    ...
                    <el-table-column property="customName" label="交易方" align="center"></el-table-column>
                    <el-table-column property="unreceiveMoney" label="金额" align="center">
                        <template slot-scope="scope">
                            <span>￥：{{scope.row.showMoney}}</span>
                        </template>
                    </el-table-column>
                    <el-table-column property="simpleSaleDate" label="时间" align="center"></el-table-column>
                </el-table>
            </div>
        </div>
    </div>
    ...
    export default {
        data() {
            return {
                ...
                unDealAmount:this.$route.params.unDealAmount,//未收款
                loading:true,
                isLoading:false,
        ...
        methods:{
            model(n){
                ...
                if(n == 0){
                    ...
                    let saleId = $(".inputcheckbox[name='radio']:checked").val()
                    if(!saleId){
                    ...
                    this.$confirm('确定是否关联？', '提示', {
                        ...
                        this.$router.push({ name: 'newSalePayment',
                            params:{saleId:saleId,advanceId:this.advanceId}})
                    }).catch(() => {
                        this.$message({
                            type: 'info',
                            message: '已取消'
...
//                console.log(headerHeight);
        ...
        created(){
            ...
            var url = addUrl.addUrl('linkedSale')
...
//                    console.log(response);
                    ...
                    for(let i in data.list){
                        data.list[i].showMoney =number.number(data.list[i].unreceiveMoney)
                    }
