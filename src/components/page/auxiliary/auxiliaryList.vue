    ...
    <div v-loading.fullscreen.lock="loading">
        <div class="w cf">
            <div class="top">
                ...
                <router-link to="/auxiliary/newAuxiliary" class="sub">新建辅助业务</router-link>
                <router-link to="/auxiliary/auxiliaryHistoryList" class="sub2">历史记录</router-link>
            </div>
        </div>
        <div class="w">
            <div class="left" :style="{height:screenHeight}">
                <el-table :data="tableData" class="blueList">
                    ...
                    <el-table-column prop="auditFlg" label="状态" sortable align="center">
                        <template slot-scope="scope">
                            ...
                            <span v-else-if="scope.row.auditFlg == 4">
                                <span v-if="scope.row.subjectCode == 1122 ||
                                scope.row.subjectCode == 122101 ||
                                scope.row.subjectCode == 122102 ||
                                scope.row.subjectCode == 122103">
                                    <router-link :to="{name:'payments',params:{debitId:scope.row.idString}}" class="see">收款</router-link>
                                </span>
                                <span v-else-if="scope.row.subjectCode == 2202 ||
                                scope.row.subjectCode == 224101 ||
                                scope.row.subjectCode == 224102 ||
                                scope.row.subjectCode == 224199">
                                    <router-link :to="{name:'payments',params:{debitId:scope.row.idString}}" class="see">付款</router-link>
                                </span>
                                <span v-else-if="scope.row.subjectCode == 1123">
                                    <router-link :to="{name:'linkedPurchase',
                                    params:{debitId:scope.row.idString,
                                            relationIdString:scope.row.relationIdString,
                                            subjectName:scope.row.subjectName,
                                            relationName:scope.row.relationName,
                                            amount:scope.row.amount,
                                            unDealAmount:scope.row.unDealAmount,
                                            }}" class="see">关联采购单</router-link>
                                </span>
                                <span v-else-if="scope.row.subjectCode == 2203">
                                    <router-link :to="{name:'linkedSale',
                                    params:{debitId:scope.row.idString,
                                            relationIdString:scope.row.relationIdString,
                                            subjectName:scope.row.subjectName,
                                            relationName:scope.row.relationName,
                    ...
                    <el-table-column label="操作" width="80px" align="center">
                        <template slot-scope="scope">
                            <span class="operation">
                                <router-link :to="{name:'seeAuxiliary',params:{debitId:scope.row.idString,currentPage:currentPage}}" class="see">
                                    <i v-if="scope.row.auditFlg == 1" class="icon iconfont icon-shuru blue"></i>
                                    <i v-else class="icon iconfont icon-kanguo blue"></i>
                                </router-link>
                            </span>
                        </template>
                    </el-table-column>
